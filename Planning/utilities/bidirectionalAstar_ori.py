...
class bidirectionalAstarPlanner(AstarPlanner):
    ...
    def plan(self, start, end):
        ...
        while open_list_forward and open_list_backward:
            ...
            for node in self.visited_backward:
                if current_forward.state == node.state:
                    # path found
            ...
            self.visited_forward.add(current_forward)
            self.visited_backward.add(current_backward)
            if self.visualization:
            ...
            for neighbor in neighbors_f:
                if self.is_valid_point(neighbor) and neighbor not in [x.state for x in self.visited_forward]:
            ...
            for neighbor in neighbors_b:
                if self.is_valid_point(neighbor) and neighbor not in [x.state for x in self.visited_backward]:
